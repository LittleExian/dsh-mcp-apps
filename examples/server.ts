...
let host:AppHost
const connection=await connectServer({serverName:'demo',transport:'stdio',command:process.execPath,args:[fileURLToPath(new URL('./mcp-server.js',import.meta.url))],env:{}},10_000,()=>host?.invalidate('demo'))
host=new AppHost([connection])
...
const server=createServer(async(req,res)=>{
  ...
  try {
    ...
    if(req.method==='GET'&&path==='/bootstrap') {
      const execution=await host.execute('demo','show-counter',{initial:4})
      const tool=host.listUi()[0]!
      return send(200,{tool,block:{kind:'tool-result',callId:'demo-call',call:{name:tool.publicName,argsRaw:'{"initial":4}'},content:execution.result.content,meta:{wiseMcpApp:{callToken:execution.callToken,publicName:tool.publicName}}}})
    }
